...
router.post("/add", async (req, res) => {
  try {
    const { name, code, type, course, semester, department } = req.body;

    const existing = await Subject.findOne({ code, course, department, semester });
    if (existing) {
      return res.status(400).json({ message: "Yeh subject already exist karta hai" });
    }

    const subject = new Subject({ name, code, type, course, semester, department });
    await subject.save();
    res.json({ message: "Subject add ho gaya", subject });
  } catch (error) {
    console.error("Subject add error:", JSON.stringify(error, null, 2));
